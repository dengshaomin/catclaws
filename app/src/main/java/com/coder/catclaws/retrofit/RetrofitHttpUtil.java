...
//        if (strs != null && strs.length > 1) url += "?" + strs[1];
    ...
    private void checkParam(Map<String, String> param) {
        ...
        if (!param.keySet().contains("xToken")) {
            param.put("xToken", UserManager.getInstance().getToken());
        }
...
//        Cache cache = new Cache(cacheFile, 1024 * 1024 * 50);
        Interceptor cacheInterceptor = new Interceptor() {
            ...
            public Response intercept(Chain chain) throws IOException {
                ...
                if (!AppUtil.isNetworkConnected(MyApplication.applicationContext)) {
                    request = request.newBuilder()
                            .cacheControl(CacheControl.FORCE_CACHE)
                            .build();
                } else if (AppUtil.isNetworkConnected(MyApplication.applicationContext)) {//网络可用
...
//                    Log.d("OkHttp", "网络可用请求拦截");
                ...
                if (AppUtil.isNetworkConnected(MyApplication.applicationContext)) {//如果网络可用
